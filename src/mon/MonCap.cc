...
void MonCapGrant::expand_profile_mon(const EntityName& name) const
{
  ...
  if (profile == "bootstrap-osd") {
    profile_grants.push_back(MonCapGrant("config-key put"));
    profile_grants.back().command_args["back"] = StringConstraint(
      StringConstraint::MATCH_TYPE_PREFIX, "dm-crypt/osd");
    ...
    profile_grants.back().command_args["entity"] = StringConstraint(
      StringConstraint::MATCH_TYPE_PREFIX, "osd.");
    profile_grants.back().command_args["caps_mon"] = StringConstraint(
      StringConstraint::MATCH_TYPE_EQUAL, "allow profile osd");
